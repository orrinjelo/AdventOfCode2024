...
pub fn visualize_00(_input: Vec<String>) {
    let image_file = File::create("problem00.gif").unwrap();
    let color_map = &[0xFF, 0xFF, 0xFF, 0, 0, 0];
    let size_x: u16 = 400;
    let size_y: u16 = 400;
    let mut encoder = Encoder::new(image_file, size_x, size_y, color_map).unwrap();
    encoder.set_repeat(Repeat::Infinite).unwrap();

    let grid = vec![vec![0u8; size_x as usize]; size_y as usize];
    ...
    info!("If there was a visualization, you'd experience it here.");
}
...
mod tests {
    ...
    /// For a mass of 14, divding by 3 and rounding down still yields 4, so the fuel required is also 2.
